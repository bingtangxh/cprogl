...
void DestroyQueue(SeQueue *Q) //依次输出队列中元素值，并释放空间。
{
    ...
    while(Q->quelen)
    {
        current=(Q->rear-Q->quelen)+1;
        if(Q<=0) current+=MAXSIZE;
        printf("%d ",Q->elem[current]);
        Q->quelen--;
    }
    free(Q);
}
