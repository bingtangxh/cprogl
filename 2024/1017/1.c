...
int main()
{
    int square1[MAXSIZE][MAXSIZE]={0},square2[MAXSIZE][MAXSIZE]={0};
    int a1,b1,a2,b2,a_max,b_max,len,a_current,b_current,value_current,a_maxr=0,b_maxr=0,slen_current;
    ...
    {
        getchar();
        scanf("%d,%d,%d",&a_current,&b_current,&value_current);
        getchar();
    ...
    for(int ia=0;ia<=a_max;ia++)
    {
        for(int ib=0;ib<=b_max;ib++)
        {
            ...
            if(square1[ia][ib]!=0)
            {
                ...
                slen_current=sprintf(result_current,"(%d,%d,%d)",ia,ib,square1[ia][ib]);
